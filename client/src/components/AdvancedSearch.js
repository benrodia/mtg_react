...
export default connect(
	...
	({
		...
		const inc = inc =>
			colors
				.map((c, i) => inc === c && COLORS("symbol")[i].toLowerCase())
				.filter(c => !!c)
				.join("")

		const C = `c:${inc(true)}${all ? "&c:m" : ""}${
			only ? `&-c:${inc(false)}` : ""
		}`
		const T = types.length
			? types.map(t => `t:${t.toLowerCase()}`).join("&")
			: ""
		const CMC = cmcOp !== "any" ? `cmc${cmcOp}${cmc}` : ""
		const R = rarity !== "any" ? `r:${rarity}` : ""
		const Q = nameEntry.length ? `${nameEntry}` : "a"
		const F = legalOnly && format !== "casual" ? `f:${format}` : ""
		const O = `order:edhrec prefer:oldest`
		const query = [Q, O, F, T, C, CMC, R]
			.filter(f => f.length)
			.map(m => m) //encodeURIComponent
			.join("&")
		...
		useEffect(
			_ => {
				axios
					.get(base + query)
					.then(res => {
						console.log("req", base + query, res.data.data)
						setResults(paginate(res.data.data.slice(0, maxRes), pageLimit))
						setPage(0)
					})
					.catch(err => {
						console.error(err)
			...
			return arr.length ? (
				<div className="block mini-spaced-col">
					<div className="bar even">
						<button
							className="smaller-button warning-button icon-cancel"
							onClick={_ => changeAdvanced({text: []})}
						/>
					...
					<div className="bar mini-spaced-bar">
						{arr.map((t, i) => (
							<span
								...
								onClick={_ =>
									changeAdvanced({[title]: arr.filter(tx => tx !== t)})
								}>
								{titleCaps(t)}
							</span>
						))}
		...
		const allTypes = [...CARD_TYPES, ...SUB_TYPES].filter(
			T => !types.includes(T)
		)
					...
					<div className="terms col">
						...
						<div className="bar even">
							...
							<button
								...
								onClick={_ => {
									changeAdvanced({
										text: [...text, ...textEntry.trim().split(" ")],
									})
									setTextEntry("")
								}}>
								Add Terms
				...
				<div className="bar block spaced-bar">
					...
					<div className="filter-colors">
						...
						<div className="bar even mini-spaced-bar">
							<div className="bar even">
								<button
									className={`small-button ${all && "selected"}`}
									onClick={_ => changeAdvanced({all: !all})}>
									Each
								</button>
								<button
									className={`small-button ${only && "selected"}`}
									onClick={_ => changeAdvanced({only: !only})}>
									Only
								</button>
							</div>
							<div className="bar-even">
							...
							<CardControls key={uuidv4()} card={c}>
								{BOARDS.map(B => (
									<button
										...
										{B} (
										{
											list.filter(l => l.name === c.name && l.board === B)
												.length
										}
										)
									</button>
