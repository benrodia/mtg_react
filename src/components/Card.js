...
})=>{return{page,sleeve,sortBy,showTypes,showPrice}},null)
(({
	...
	const price = <p className='prices' style={{display:page==='Build'&&(USD||TIX)&&showPrice||'none'}}><b>{USD||'???'}</b> - <b>{TIX||'???'}</b></p>

	return cardHeadOnly
		? <div className={`card-head`}>
			<h4 className='name'>{name}</h4>
			<p className='types'>{subType(type_line)} {formatManaSymbols(mana_cost)}</p>
			{price}
		  </div>
		: <>
			<img 
