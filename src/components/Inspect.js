...
export default connect(({main:{page},deck:{list}})=>{return{page,list}},actions)
(({back,page,list,inArea,options,card,openModal,addCard,newMsg,changeCard})=> {
	...
	const shiftList = by => {if (inArea&&ind) {
	...
	useEffect(_=>{
		const keyEvent = e => {
			if (cooledDown) {	
				...
				switch (e.key) {
					case 'ArrowLeft': return shiftList(-1)
					case 'ArrowRight': return shiftList(1)
				}
			}
		}
	...
	const inner = options==='Add'?
		<div className="options">
		{BOARDS.map(B=><button key={B} className='small-button' onClick={_=>{
				addCard(card,B)
				newMsg(`Added ${card.name} to ${B}board`,'success')
			}}>{B} ({list.filter(l=>l.name===card.name&&l.board===B).length})</button>
		)}</div>
	: options==='Move' ?
		page==='Build'?
		<div className="options">
		...
		return <button key={B} className='small-button' onClick={_=>changeCard(card,{board:B})}>
			{B} ({cards.length})
		</button>
		})}</div>
		:page==='Test'?
	...
	const header = 
			<div className="header">
				<div className="inspect-nav">
					...
					<button style={{display:!list&&'none'}} onClick={_=>shiftList(-1)}>{'<'}</button>
					<button style={{display:!list&&'none'}} onClick={_=>shiftList(1)}>{'>'}</button>
				</div>
				{!options?null:inner}
			</div>
	
	return <div key={'inspectTarget'+name} className='inspect-container'>
		{header}
		<div className="info">			
