...
export default connect(({
  deck:{name,format,list},
  settings:{userName}
}) => {return {name,format,list,userName}},actions)
(({accept,name,userName,list,format,openModal})=> {

  const fn = titleCaps(format.replace(/\s+/g, '_'))+"_"+
  ...
  const [fileExt,setFileExt] = useState('.txt')
    ...
    return <div className="download-box">
      <h3>File Name: <input type="text" value={fileName} onChange={e=>setFileName(e.target.value)}/></h3>
      <h3>Preview File:</h3>
      ...
      <a onClick={_=>openModal(null)} download={`${fileName}${fileExt}`} 
