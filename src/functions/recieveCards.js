...
export default function receiveCards(deck,sleeve,isToken) {
  ...
  return list.map((c,i)=> Object.assign(c,{
      ...
      flipped: false,
      sickness: true,
      mana_source: !MANA.source(c)?false
      :MANA.any(c)?COLORS('symbol').map(co=>co===5?0:NUM_FROM_WORD(c.oracle_text))
